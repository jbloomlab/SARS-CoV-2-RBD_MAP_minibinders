...
rule make_summary:
    ...
    run:
        ...
        with open(output.summary, 'w') as f:
            ...
            8. Map escape profiles to ``*.pdb`` files using [this notebook]({path(input.output_pdbs)})
            
            9. Output a list of RBD structural contacts for each antibody with a high-resolution structure using [this notebook]({path(input.structural_contacts)})
